...
def generate_default_adv_example(sess, cleverhans_model, attack_type, x_op):
  ''' Return adversarial example '''
  if attack_type == 'fgsm':
    attack_op = FastGradientMethod(cleverhans_model, sess=sess)
  elif attack_type == 'pgd':
    attack_op = ProjectedGradientDescent(cleverhans_model, sess=sess)
  adv_x_op = attack_op.generate(x_op)
  return adv_x_op
def evaluate_default_tf(sess,test_loader, tf_model_fn, adv_x_op, x_op):
  adv_preds_op = tf_model_fn(adv_x_op) 
  # Run an evaluation of our model against fgsm
  total = 0
  correct = 0
...
## MomentumIterativeMethod
  adv_x_op = generate_default_adv_example(sess,cleverhans_model, 'fgsm', x_op)
  # evaluate_tf(sess, test_loader, tf_model_fn, attack_params, adv_x_op, x_op)
  evaluate_default_tf(sess, test_loader, tf_model_fn, adv_x_op, x_op)
